...
//-------------------------------------------------------------
  ...
  {  
	  ...
	  if(tempfd != null)
	  {
          ...
          else
          {
              ...
              if (tempfd.pageno.pid>8){
                for(int j=0;j<topKref.length;j++){
                    if (topKref[j][0]==tempfd.pageno.pid){
                        topKref[j][2]=topKref[j][2]+1;
                    }
                }
	  ...
	  else
	  {
          ...
          int[] arr12= replacer.pickVictim();
          int i =arr12[0];
          int secondchance=arr12[1];
          //if buffer pool is full throws an Exception("Buffer pool exceeded")
          if(i < 0)
        	  throw new IllegalStateException("Buffer pool exceeded");
                
          tempfd = frametab[i];
          if(tempfd.pageno.pid>8){                  // this condition updates the reference count of victim page
            for(int j=0;j<topKref.length;j++){
                if(topKref[j][0]==tempfd.pageno.pid){
                    topKref[j][3]=topKref[j][3]+1;
                    topKref[j][4]=topKref[j][4]+secondchance;
                    }
                }
            }
...
// some BHR code may go here
          ...
          if (pageno.pid>8){
            for(int j=0;j<topKref.length;j++){
                if (topKref[j][0]==pageno.pid){
                    topKref[j][1]=topKref[j][1]+1;
...
// some BHR code may go here
 ...
 //Function to sort by column 
    ...
    public void printBhrAndRefCount(){ 
        int aggregatePagehit=0;
        int aggregateLoad=0;
        float aggPagehit;
        float aggLoad;
        for(int j=0;j<topKref.length;j++){
            aggregatePagehit=aggregatePagehit+topKref[j][2];
            aggregateLoad=aggregateLoad+topKref[j][1];
        }
        float BHR;
        System.out.println("Aggregate Page HIT  "+ aggregatePagehit);
        System.out.println("+----------------------------------------+");
        System.out.println("Aggregate page Load  "+aggregateLoad);
        System.out.println("+----------------------------------------+");
        aggPagehit=(float)aggregatePagehit;
        aggLoad=(float)aggregateLoad;
        BHR=aggPagehit/aggLoad;
        System.out.println("Buffer hit ratio  "+BHR);
        System.out.println("+----------------------------------------+");
        //sortbyColumn(topKref, 2);
        System.out.print("[Page.No]   ");
        System.out.print("[No.of Page Loads]  ");
        System.out.print("[No.of Page Hits]   ");
        System.out.print("[No.of Victim Page Referenced]   ");
        System.out.println("[No.of Pages second chance]");
        for(int j=0;j<topKref.length;j++){
            
            System.out.print(topKref[j][0]+"                ");
            System.out.print(topKref[j][1]+"                    ");
            System.out.print(topKref[j][2]+"                        ");
            System.out.print(topKref[j][3]+"                                ");
            System.out.println(topKref[j][4]);
        }
